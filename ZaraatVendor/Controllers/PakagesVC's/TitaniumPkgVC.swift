...
class TitaniumPkgVC: UIViewController, IndicatorInfoProvider {
    func indicatorInfo(for pagerTabStripController: PagerTabStripViewController) -> IndicatorInfo {
        return IndicatorInfo(title: "Titanium")
    }
   ...
   @IBOutlet weak var tblView: UITableView!
    ...
    @IBAction func upgradeAction(_ sender: UIButton) {
        ...
        let vc =  storyBoard.instantiateViewController(withIdentifier: "PkgDetailVC") as? PkgDetailVC
            vc?.pkgid = (packages?[5].package_id)!
    ...
    extension TitaniumPkgVC : UITableViewDelegate,UITableViewDataSource {
        ...
        func numberOfSections(in tableView: UITableView) -> Int {
              ...
              func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
                ...
                cell?.lblprice.text = "PKR " +  (packages?[5].price ?? "")   + " / Year"
                cell?.lblshowcase.text =  (packages?[5].showcase_products ?? "")
                cell?.lblAccounts.text =  (packages?[5].sub_accounts ?? "")
                cell?.lblValidity.text =  (packages?[5].account_validity ?? "")
                cell?.lblads.text =  "\(packages?[5].ads_number ?? 0)"
                   
                if packages?[5].conference_workshop ==  1 {
                   cell?.lblworkshops.text = "Yes"
                   } else {
                       cell?.lblworkshops.text =  "No"
                   }
                   
                if packages?[5].inquiries == 1 {
                       cell?.lblEnquiries.text =  "Yes"
                   } else {
                       cell?.lblEnquiries.text =  "No"
                   }
                   
                if packages?[5].reporting ==  1 {
