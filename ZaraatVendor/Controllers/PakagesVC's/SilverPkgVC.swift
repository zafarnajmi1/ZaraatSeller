...
class SilverPkgVC: UIViewController , IndicatorInfoProvider{
    
    func indicatorInfo(for pagerTabStripController: PagerTabStripViewController) -> IndicatorInfo {
        return IndicatorInfo(title: "Silver")
    }
    ...
    @IBAction func upgradeAction(_ sender: UIButton) {
        ...
        let vc =  storyBoard.instantiateViewController(withIdentifier: "PkgDetailVC") as? PkgDetailVC
            vc?.pkgid = (packages?[1].package_id)!
...
extension SilverPkgVC : UITableViewDelegate,UITableViewDataSource {
    ...
    func numberOfSections(in tableView: UITableView) -> Int {
          ...
          func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
            ...
            cell?.lblprice.text = "PKR " +  (packages?[1].price ?? "")   + " / Year"
            cell?.lblshowcase.text =  (packages?[1].showcase_products ?? "")
            cell?.lblAccounts.text =  (packages?[1].sub_accounts ?? "")
            cell?.lblValidity.text =  (packages?[1].account_validity ?? "")
            cell?.lblads.text =  "\(packages?[1].ads_number ?? 0)"
               
            if packages?[1].conference_workshop ==  1 {
               cell?.lblworkshops.text =  "Yes"
               } else {
                   cell?.lblworkshops.text =  "No"
               }
               
            if packages?[1].inquiries == 1 {
                   cell?.lblEnquiries.text =  "Yes"
               } else {
                   cell?.lblEnquiries.text =  "No"
               }
               
            if packages?[1].reporting ==  1 {
