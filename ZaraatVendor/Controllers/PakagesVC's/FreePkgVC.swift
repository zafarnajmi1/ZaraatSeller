...
class FreePkgVC: UIViewController , IndicatorInfoProvider{
    
    func indicatorInfo(for pagerTabStripController: PagerTabStripViewController) -> IndicatorInfo {
        return IndicatorInfo(title: "Free")
    }
...
//            vc?.pkgid = (packages?[0].package_id)!
...
//    }
    ...
    extension FreePkgVC : UITableViewDelegate,UITableViewDataSource {
        ...
        func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
            ...
            cell?.lblprice.text = "PKR " +  (packages?[0].price ?? "")   + " / Year"
            cell?.lblshowcase.text =   (packages?[0].showcase_products ?? "")
            cell?.lblAccounts.text =  (packages?[0].sub_accounts ?? "")
            cell?.lblValidity.text =  (packages?[0].account_validity ?? "")
            cell?.lblads.text =  "\(packages?[0].ads_number ?? 0)"
            
            if packages?[0].conference_workshop ==  1 {
            cell?.lblworkshops.text =  "Yes"
            } else {
                cell?.lblworkshops.text =   "No"
            }
            
            if packages?[0].inquiries == 1 {
                cell?.lblEnquiries.text =  "Yes"
            } else {
                cell?.lblEnquiries.text =  "No"
            }
            
            if packages?[0].reporting ==  1 {
