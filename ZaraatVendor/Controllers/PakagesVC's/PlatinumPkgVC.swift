...
class PlatinumPkgVC: UIViewController, IndicatorInfoProvider {
    ...
    @IBAction func upgradeAction(_ sender: UIButton) {
        ...
        let vc =  storyBoard.instantiateViewController(withIdentifier: "PkgDetailVC") as? PkgDetailVC
            vc?.pkgid = (packages?[3].package_id)!
    ...
    extension PlatinumPkgVC : UITableViewDelegate,UITableViewDataSource {
        ...
        func numberOfSections(in tableView: UITableView) -> Int {
             ...
             func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
                ...
                cell?.lblprice.text = "PKR " +  (packages?[3].price ?? "")   + " / Year"
                cell?.lblshowcase.text =  (packages?[3].showcase_products ?? "")
                cell?.lblAccounts.text =  (packages?[3].sub_accounts ?? "")
                cell?.lblValidity.text =  (packages?[3].account_validity ?? "")
                cell?.lblads.text =  "\(packages?[3].ads_number ?? 0)"
                   
                if packages?[3].conference_workshop ==  1 {
                   cell?.lblworkshops.text =  "Yes"
                   } else {
                       cell?.lblworkshops.text =  "No"
                   }
                   
                if packages?[3].inquiries == 1 {
                       cell?.lblEnquiries.text =  "Yes"
                   } else {
                       cell?.lblEnquiries.text =  "No"
                   }
                   
                if packages?[3].reporting ==  1 {
